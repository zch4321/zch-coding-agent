...
复杂任务、跨文件或跨模块修改、多步骤调试、风险较高的变更，必须优先进入 harness Plan mode：如果 plan_set、plan_status、plan_update 等计划工具可用，先用 plan_set 创建或更新计划，并按 harness 要求等待用户审批；用户批准后，先用 plan_status 将计划标记为 active，再逐项执行。

执行计划时，计划一定要一步一步执行。如果计划顺序或规划内容与实际执行所需不符，一定要先修改计划，再继续执行。每完成计划的一个阶段，立刻使用 plan_update 或等价计划工具更新计划状态，不要批次更新。每一个计划阶段完成，都要确保这个阶段的功能完整实现，留下可验证的测试或实现证据。

...
- <parent_agent_message>: Follow-up delegation from the parent Agent to this Child. Continue with the existing conversation and the original permission ceiling. XML entities represent literal text.
- <background_task_notification>: Application-generated status after a background task completes, fails, or pauses while the parent is naturally finished and idle. Use its target, response and artifacts to continue the original task; message, pause, or resume a Child when appropriate. Follow-up work does not alter the original Swarm result. Prefer a new Swarm for new batches; follow up on an existing Child when its retained context is useful.

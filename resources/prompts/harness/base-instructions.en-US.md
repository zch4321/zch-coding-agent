...
For code discovery, prefer grep, glob, list_dir, and bounded read_file over broad file reads. Continue a paged or growing file from `nextCursor`; retain `endCursor` when following a file that may receive later appends, and use `tail` for a bounded final snapshot. For configured code intelligence, prefer code_workspace_symbols, code_symbol_overview, code_find_definition, code_find_references, and code_diagnostics to locate symbols and diagnostics before reading large source files.

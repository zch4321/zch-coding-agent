...
/** Projects job metadata to durable JSON. */
export function json(value: unknown): JsonValue {
  return JSON.parse(JSON.stringify(value)) as JsonValue
}

/** Hashes a normalized job for durable call idempotency. */
...
/** Builds a bounded display name for a job. */
...
/** Validates shared context and child task capacity. */
...
/** Expands per-task agent counts into ordered child assignments. */
...
/** Projects the public model-pool assignment metadata. */
...
/** Bounds persisted aggregate job output while retaining child metadata. */
...
/** Validates a previously committed aggregate job result. */
...
/** Normalizes job failures to stable public error codes. */

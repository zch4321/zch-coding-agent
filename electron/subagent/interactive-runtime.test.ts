...
class ConversationProvider extends ScriptedProviderHarness {
  ...
  readonly followupGate = new Promise<void>((resolve) => {
    this.releaseFollowup = resolve
  })
  #step = 0
  constructor(
    private readonly restoring = false,
    private readonly swarm = false,
  ) {
    super()
  }
  ...
  async *run(
    ...
    if (!request.toolDefinitions.some((tool) => tool.name === 'subagent_run')) {
      ...
      if (this.swarm && !this.restoring && this.children.length === 2)
    ...
    if (step === 1) {
      yield this.restoring
        ...
        : this.swarm
          ? call(
              'swarm_run',
              {
                sharedContext: 'Check independently.',
                tasks: [
                  {
      ...
      this.target = this.restoring
        ? this.swarm
          ? result.tasks[0].children[0].target
          : result.tasks[0]
        : result.target
      yield this.restoring
        ? call(
      ...
      if (this.swarm) this.target = result.targets[0].children[0].target
      yield call(
        'subagent_send_message',
        {
          target: this.target!,
...
it.each([false, true])(
  'reuses one hidden Session and preserves original Swarm results across restart (swarm=%s)',
  async (swarm) => {
    ...
    if (swarm) {
      let provider = config.getPublicConfig().models.providers[0]!
      await config.update({
        version: 1,
        kind: 'provider',
      ...
      provider = config.getPublicConfig().models.providers[0]!
      await config.update({
        version: 1,
        kind: 'model-pool',
        value: {
    ...
    const first = new ConversationProvider(false, swarm)
    const options = {
      configStore: config,
    ...
    try {
      ...
      if (swarm) {
        await waitFor(() => first.children.length === 2)
        const groups = await backend.agentExecutions.list({
          parentSessionId: sessionId,
        })
        expect(groups.records).toHaveLength(1)
        expect(groups.records[0]).toMatchObject({
          status: 'completed',
          hasActiveChildren: true,
          agentCounts: { total: 1, completed: 1, running: 0 },
        })
        first.releaseFollowup()
      }
      ...
      const secondContext = JSON.stringify(
        first.children[1]!.normalizedMessages,
      )
      ...
      const saved = (
        await backend.coordinator.query((reader) =>
          reader
            .prepare(
              "SELECT child_session_id, child_run_id, parent_run_id FROM subagent_executions WHERE kind = 'subagent' ORDER BY rowid",
            )
            .all(),
        )
      ).value
      ...
      const groupResult = (
        await backend.coordinator.query((reader) =>
          reader
            .prepare(
              "SELECT result_json FROM subagent_executions WHERE kind='swarm'",
            )
            .get(),
        )
      ).value
      if (swarm) {
        expect(JSON.stringify(groupResult)).toContain('child answer 1')
        expect(JSON.stringify(groupResult)).not.toContain('child answer 2')
      }
      await backend.dispose()
      const restored = new ConversationProvider(true, swarm)
      ...
      if (continued.outcome !== 'started')
        throw new Error('Run did not restart')
      await backend.runtime.services.sessions.waitForRunSettled(
        sessionId,
        continued.runId,
      )
      ...
      expect(
        JSON.stringify(restored.children[0]!.normalizedMessages),
      ).toContain('child answer 2')
      expect(
        (

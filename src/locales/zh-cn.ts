const zhCN = {
  ...
  settings: {
    ...
    modelSettings: '模型配置',
    modelSettingsHint:
      '右侧是该 Provider 已启用的模型池，只启用的模型会出现在对话、Provider 和未来 Swarm 的模型选择中；修改会自动保存。',
    ...
    selectMainModel: '先启用模型',
    selectMainModelHint: '请先在下方模型池中启用模型，再选择主模型。',
  ...
  modelPool: {
    title: '模型池',
    hint: '为后续 Swarm 调度配置可复用模型。能力等级只读取 Provider 的模型标注，整组修改一次性保存。',
    add: '添加模型',
    save: '保存模型池',
    empty: '模型池为空。添加一个已标注能力等级的 Provider 模型。',
    noEligibleModels:
      '没有可添加的模型。请先在“模型服务”中启用模型并标注能力等级。',
    entryId: '条目名称',
    entryIdPlaceholder: '例如 strong-coder',
    idRequired: '条目名称不能为空。',
    idDuplicate: '条目名称必须唯一。',
    enabled: '启用',
    provider: 'Provider',

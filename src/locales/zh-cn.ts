const zhCN = {
  ...
  settings: {
    ...
    baseUrl: '基础 URL',
    mainModel: '主模型',
    contextOverride: '上下文窗口（可选覆盖）',
    contextOverrideHint:
      '用于本地上下文预算和自动压缩；留空时使用模型目录、内置资料或全局默认值。',
    outputOverride: '最大输出 Token（可选覆盖）',
    outputOverrideHint:
      '限制单次生成并为输出预留上下文；留空时使用模型目录、内置资料或安全默认值。',
    ...
    modelRefreshCredentialHint: '先在下方保存 API Key，才能刷新模型目录。',
    modelRefreshUnsavedHint: '先用下方按钮保存 Provider 修改，再刷新模型目录。',
    saveProvider: '保存模型服务',

...
采用现有 UTF-8 bytes/token 估算器和冻结模型的窗口容量。分类包括协议内容包装，不包括 model、max_tokens、stream 等控制参数；类别之和等于本地上下文总估算，不强行对齐 Provider 的 prompt usage。
...
右侧“用量”页签分为“当前上下文”和“用量明细”。前者提供分段条、分类大小、比例和来源展开；后者切换整个会话或当前/最近运行，按主对话、子代理、工具审批、上下文压缩、标题生成分组，展开模型或子任务。字段缺失显示破折号，不添加历史覆盖或准确性提示文案。

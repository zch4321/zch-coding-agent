...
> 当前脚手架 `electron/preload.ts` 直接暴露了通用 `ipcRenderer.{on,off,send,invoke}`，与本设计冲突，进入功能开发前必须替换。

### 1.3 当前脚手架差距
- `electron/preload.ts` 暴露通用 IPC，`electron-env.d.ts` 也把完整 `IpcRenderer` 暴露给 renderer；需改为 `AgentApi` 白名单类型。
- `electron/main.ts` 仍有模板测试事件，且未显式配置 CSP、导航/新窗口限制、IPC sender 校验和应用退出资源清理。
- `package.json` 只有 Vue 基础依赖，Electron 版本为 30；Naive UI、Pinia、LLM SDK、schema validator、markdown sanitizer、node-pty 等尚未选定/安装。
- `electron-builder.json5` 的 `appId/productName` 仍是占位值，native module rebuild/unpack 和 Windows 签名策略尚未定义。
- 当前没有测试 runner、lint、依赖锁定和 CI，§15 的安全/协议测试尚无承载环境。
...
│   ├── preload.ts                 # contextBridge 窄接口（需替换脚手架通用桥）
│   ├── ipc/
│   │   ├── index.ts               # 注册所有 invoke handler + 事件
│   │   ├── validate-sender.ts      # sender / frame / origin 校验
│   │   └── validators.ts           # payload runtime schema 校验
│   ├── agent/
│   │   ├── loop.ts                # Agent Loop 状态机（§3）
│   │   ├── context.ts             # 会话/工作区/消息历史 持有
│   │   ├── context-budget.ts       # token/工具输出预算与压缩策略
│   │   ├── runtime.ts              # 与 BrowserWindow 解耦的运行时边界
│   │   └── interrupt.ts           # 中断控制（AbortController）
│   ├── llm/
│   │   ├── provider.ts            # LLMProvider 接口（§4.1）
│   │   ├── continuation.ts         # ProviderContinuationState（§4.2）
│   │   ├── deepseek.ts            # [MVP] DeepSeek Provider
│   │   └── registry.ts            # provider 注册表 / 按配置实例化
│   ├── tools/
│   │   ├── registry.ts            # 工具注册表（schema + handler）
│   │   ├── types.ts               # Tool / effects / ToolResult 约定
│   │   ├── execution.ts           # ApprovedToolCall + timeout/abort/输出上限
│   │   ├── fs/
│   │   │   ├── read.ts write.ts edit.ts delete.ts
│   │   ├── search/
│   │   │   ├── list.ts glob.ts grep.ts
│   │   ├── command/
│   │   │   ├── run.ts             # run_command
│   │   │   └── process-tree.ts    # 平台相关进程树终止
│   │   ├── terminal/
│   │   │   ├── pty-pool.ts        # node-pty 实例池（§8）
│   │   │   ├── open.ts send.ts read.ts list.ts close.ts resize.ts
│   │   ├── skills/
│   │   │   └── read-skill.ts      # read_skill 工具（只读）
│   │   └── path-guard.ts          # 工作区边界检测（§9 共用）
│   ├── skills/
│   │   ├── manager.ts             # 扫描目录、解析 frontmatter、构建摘要
│   │   ├── installer.ts           # 链接下载 + 上传安装（写入用户目录）
│   │   └── types.ts               # Skill / SkillSummary 类型
│   ├── mcp/                       # [MVP 之后] MCP 客户端
│   │   ├── client.ts              # 协议握手、list/call tool 转发
│   │   ├── registry.ts            # 把 MCP server 工具注册进 ToolRegistry
│   │   ├── transport/
│   │   │   ├── stdio.ts           # spawn 子进程
│   │   │   ├── streamable-http.ts # Streamable HTTP（预留）
│   │   │   └── legacy-sse.ts      # 旧 HTTP+SSE 兼容（预留）
│   │   └── lifecycle.ts           # server 进程管理（启动/重启/清理）
│   ├── permission/
│   │   ├── pipeline.ts            # 审批主管线（§9）
│   │   ├── invariants.ts           # schema / 资源归属 / workspace 契约
│   │   ├── policy-engine.ts        # 确定性 allow/deny/review
│   │   ├── command-signals.ts      # 命令黑名单/风险信号，不作完整 shell parser
│   │   ├── sensitive-data.ts       # 可选的外发 warn/confirm 规则
│   │   ├── remembered-rules.ts     # 带 scope/expiry 的用户规则
│   │   └── approver-model.ts      # Auto 模式的审批模型调用
│   ├── plugins/
...
├── src/                           # 渲染进程 (Vue)
│   ├── main.ts App.vue
│   ├── stores/                    # Pinia（会话、权限模式、终端列表…）
│   ├── composables/               # useIpc / useAgentStream / useTerminal
│   ├── components/
│   │   ├── chat/                  # ChatView / MessageItem / ToolCallView
│   │   ├── terminal/              # TerminalView（订阅 PTY 流）
│   │   ├── diff/                  # DiffViewer
│   │   └── approval/              # ApprovalDialog
│   └── api/                       # 对 window.agentApi 的 typed wrapper
└── docs/                          # 本文档
```
...
preload 对 renderer 暴露 `window.agentApi`，每个方法绑定固定 IPC channel。回调只接收业务 payload，不暴露 Electron `IpcRendererEvent`。当前脚手架的通用 `window.ipcRenderer` 必须删除。
...
### 8.2 双向流
- **输出**：pty.onData → 写有限 scrollback + 经 `terminal:event` 推送给订阅窗口（**原始带 ANSI**，UI 着色渲染）。UI 通过 seq 检测丢片，重连时请求有限快照。
...
| 领域 | 选型 | 备注 |
|---|---|---|
| 框架 | Electron + Vite + Vue 3 + TypeScript | 当前脚手架是 Electron 30/Vite 5/Vue 3.4；实现前升级到仍受支持的 Electron 版本并锁定依赖 |
| LLM SDK | openai SDK（DeepSeek）+ 自封装 LLMProvider | 各家独立 Provider + continuation adapter |
| 终端 | node-pty | persistent PTY，需 electron-rebuild |
| 代码高亮 | Shiki | |
...
### 14.1 已知风险
- **node-pty 是 native 模块**：electron-builder 打包需 `electron-rebuild`，跨架构构建（如 arm64 mac）需注意。开发期 `npm run dev` 由 vite-plugin-electron 处理。

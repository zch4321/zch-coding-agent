...
默认与 Desktop 使用同一个应用数据目录下的 `agent.db`。可通过 `--profile-dir <目录>` 或 `ZCH_PROFILE_DIR` 选择独立 profile，CLI 参数优先；Desktop 支持同样的选择。同 profile 暂时只支持轮流运行，启动时遇到活跃 owner 返回 `PROFILE_IN_USE`，关闭该宿主后再重试。失效 owner 在确认进程已退出后回收。CLI 的 Provider、权限和其他配置只在本次内存中覆盖，不写回 Desktop 的 config/secrets。

相同 canonical workspace 复用持久 Project，任务历史与产物编号连续。模型看到稳定的项目 `workspace/tmp` 原生入口。运行时捕获在写入结束后保留 24 小时，详见[共享产物](../architecture/integrations.md#项目临时工作区与-artifact)。`--artifacts` 是当前任务的导出目录；建议每次选独立目录，导出副本由调用者保管。

## 读取结果

- Host stdout 是版本化 JSONL，诊断写 stderr，运行日志与任务输出写入 artifacts。
- `result.json` 和 runtime identity 原子写入；`artifacts.captureIndexPath` 指向当前任务的 `artifacts.json`，列出持久产物引用、原生地址、导出地址和可用性。完整捕获位于导出目录的 `artifacts/<kind>/<id>`，trace 与运行日志仍在 `runtime/`。具体字段从版本化契约读取。
...
- 不输出 workspace patch，不修改 Git index 来采集结果。退出保留 profile 的持久 SQLite、Project、Session 和编号。

...
- 页面顶部为“默认模型”分节：两个下拉——主模型（新对话的默认执行模型，会话/草稿级覆盖不受影响）与辅助模型（用于对话自动起名和自动权限审批等后台任务；默认项“跟随当前模型”即留空）。候选为所有 Provider 的已启用模型，选择即自动保存。
- 第二节为“供应商配置”，即原 Provider 管理内容；Provider id 是内部稳定标识，不在 UI 展示。
- Base URL。

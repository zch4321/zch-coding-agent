...
> 来源与方法：10 个区域并行深审（application×3、P5 FileChanges、session runtime、compact/history、providers、persistence、shared contracts、config/infra/docs）；全部 High/Medium 级发现均经主评审对照当前源码独立复核确认；门禁独立复跑。
> 与既有报告的关系：本报告**完整整合**了 [`backend-refactor-p5-code-review.md`](./backend-refactor-p5-code-review.md)（P5 增量视角，P1/P2/P3 定级）的全部发现、分析与修复顺序建议，重叠条目保留其编号交叉引用（记作 =P1-1 等）并合并双方细节；个别定级/事实差异以本报告复核结论为准（见 M-6 影响面修正、APP-6 定级说明）。整合后该增量文档保留为 P5 阶段记录，本报告为分支当前唯一全量评审基线。本报告仅评审，未修改任何代码。
...
但修复层之下仍发现 **1 个 High、11 个 Medium、约 30 个 Low**。主题分布：
...
2. **P5 文件系统执行层缺陷**：revert 的 TOCTOU 可越过 workspace 边界写/删文件（FC-1，=P1-1）、after 重验无界读文件（FC-2，=P2-1）、beforeHash 不交叉校验（FC-3，=P2-2）、权限位丢失（FC-5，=P2-3）、`UNIQUE(session_id, call_id, path)` 与压缩后 call ID 重用冲突（FC-4，=P2-5）。
3. **确认未修复的上轮 Medium**：M-5（exec 假失败真提交，实测复现）、M-6（配置重置策略，P5 新增必填字段使其成为直接样本，=P1-3）、M-7（smoke skip 顺序）、M-8（第 513 个 Project，=P2-6）、M-1（reasoning-only completion，=P2-7）、M-9（源头聚合上限，=P2-8）。
4. **幂等与一致性缺口**：请求指纹只含消息文本，同 ID 同文本不同附件/新会话参数被静默复用（REQ-1，=P2-4）；in-flight 请求 LRU 逐出后同 key 重入共享 ownerToken（APP-6，=P1-2）。

与 P5 增量报告的结论一致：**当前状态不建议作为生产切换或 P8 收尾的基线**。建议处理顺序：P6 开工前处理 FC-1、PRV-1、SR-1、APP-1、PER-1、APP-6 与 M-6 的策略决策；P6/P8 接线前处理其余 Medium 与接线相关 Low；随后补齐文件快照与请求幂等语义的端到端完整性测试。
...
P5 增量报告复跑（同一 HEAD，结论并入）：`git diff --check master...HEAD` 通过；`npm run test:e2e` 29/29 通过；`npm run test:sqlite:packaged` 在 macOS 审查机上按脚本设计 skip。两份评审均未运行 `npm run test:real`（需真实 `DEEPSEEK_API_KEY`，不属于确定性门禁）。

备注：本轮首轮复跑与 10 个评审子任务并发、机器重载，`shared/domain-state-contracts.test.ts`（5 s 默认超时）与 `electron/process/run.test.ts`（进程派生时序）各失败一次；两者单独复跑及无负载全量复跑均通过，判定为**负载敏感型 flake**，与上轮 Info「套件对慢机器偏脆」一致，建议 P10 统一处理这两处的超时/重试余量。本轮未复跑 `test:native` / `test:ripgrep`。
...
| M-1  | 未修复     | =P2-7，见第 6 节。`chat-completions-adapter.ts:355-357` 原样；`parts.minItems=1` + `TextPart.minLength=1` 使「空 text 占位」不可行，需 schema 层决策                           |
...
| M-6  | 未修复     | =P1-3，见第 6 节。P5 新增必填 `fileChangeHistoryBytes` 沿用同一 reset-only 语义。**影响面修正**：v9 配置从未发布（release 0.2.1–0.2.3 均为 master v8 产物），当前真实影响仅限分支开发机；但「新增必填 limits 字段 = 全量清配置」的模式必须在任何 v9 发布后再次加字段前解决 |
| M-7  | 未修复     | 见第 6 节。`scripts/sqlite-smoke.cjs:107-110`：`packagedElectronPath()`（缺目录即 ENOENT）仍在非 Windows skip 判断之前求值（主评审抽查确认）                                  |
| M-8  | 未修复     | =P2-6，见第 6 节。`project-service.ts:75-101` add 无容量检查，list 固定 `LIMIT 512`，契约无 project:get/分页                                                                |
| M-9  | **部分修复** | =P2-8，见第 6 节。源头仍无聚合上限；durable 组合由 H-5 恢复兜底（run 以 codec 错误失败但 session 不中毒）；**legacy 默认组合下超限附件仍使 session 内存态永久不可用直至重启**  |
...
| REQ-1 | Medium | application | 请求幂等指纹只含消息文本，忽略 selected context 与新会话语义，同 ID 被静默复用    | =P2-4，已复核   |
...
| M-6   | Medium | config      | 高版本/缺字段配置无备份整份重置、secret 孤儿；P5 新增必填字段使其成直接样本       | =P1-3           |
| M-8   | Medium | application | 第 513 个 Project 写库成功但从所有 full-list API 消失                             | =P2-6           |
| M-1   | Medium | providers   | reasoning-only / 空 content 的合法 completion 被误判为错误                        | =P2-7           |
| M-9   | Medium | session     | 合法附件组合聚合后超单 Message text 上限                                          | =P2-8           |
...
### FC-1 revert 的最终 rename/unlink 在最后一次校验之后使用词法路径，TOCTOU 可越过 workspace 边界（=P1-1）

`electron/application/file-change-filesystem.ts:31-64,88-134`（主评审抽查确认，与 P5 报告 P1-1 独立互证）：`readFileContentState()` 先从词法路径计算候选、`lstat`/`realpath` 校验后，仍把最初的 `absolutePath` 写入返回状态；`restoreFileContent` 在 `:88-93`、`:111-112`、`:122-123` 完成目标与父目录 realpath 校验后，`:113`（覆盖写 `rename(temp, absolutePath)`）与 `:128-130`（删除分支 `rename` + `unlink`）使用的仍是最初词法解析的 `absolutePath`。

外部编辑器、同步程序或拥有 workspace 写权限的本地进程可以在最后一次检查后替换目标：

- patch/delete 回滚用旧快照覆盖竞争方刚写入的内容；create 回滚删除竞争方刚创建的文件。
- 更严重：已校验的父目录可在检查后被移动并替换为指向 workspace 外的 symlink，最终 `rename`/`unlink` 会重新解析该链接，从而**在 workspace 外写入或删除文件**。

`WorkspaceAccessCoordinator` 只协调进程内写者，无法消除外部进程或文件系统对象变化带来的竞态；仅增加一次 `realpath` 复查也无法关闭检查与系统调用之间的窗口。

- 影响：破坏 workspace 路径边界不变量（AGENTS.md 明确要求保留），可越界覆盖/删除文件。窗口窄但触发源现实存在。
- 建议：将回滚实施为相对已验证目录句柄的 no-follow 原子操作（`openat`/`renameat` 风格，需原生层），或重设计为可检测、可恢复的两阶段操作并在契约中明确 best-effort 语义；不要在校验后再凭未绑定到目录句柄的字符串路径执行破坏性操作。
- 应补测试：提供可控 hook，在最后一次校验之后分别注入「目标内容变化」与「父目录换成指向 workspace 外的链接」，断言不会覆盖、删除或越界写入。
...
### PER-1 `PersistenceTransaction.exec` 允许事务控制语句，实测「假失败真提交」（=M-5）
...
### APP-6 in-flight 请求被 LRU 逐出后同 key 重入共享 ownerToken，失败方可拆毁成功方 live context（=P1-2）

`electron/application/durable-run-application-service.ts:50-90,149-202` + `live-session-context-registry.ts:57-107`：`#requests` 同时承担「进行中单飞表」与「最近请求缓存」，达到 1000 项后 `:85-88` 无条件逐出最旧项——**包括在途条目**。相同 sessionId+clientRequestId 的重试随后可再次进入 `#startNew()`；`reserveNew()` 对同一保留请求返回同一个 ownerToken，两次启动在 `SessionManager.createSession()` 的异步路径（PathGuard/MCP 初始化，`session-manager.ts:297-331`）上交叠；任一方失败分支的 `releaseOwned()` 会按共享 token 释放并关闭另一条实际成功路径创建的 live session。`#startNew()` 的失败清理（`:78`）还无条件删除 key，可能误删后续请求的 promise。

- 影响：触发门槛不低（>1000 个未完成新会话请求 + 一次重试），但一旦发生会造成用户刚创建的会话被关闭或首条消息丢失，属过载场景下的生命周期正确性破坏。定级说明见第 4 节（P5 报告 P1，本报告 Medium；P6/P8 前必须修）。
- 建议：将 pending map 与已完成 LRU 分离，绝不逐出未完成条目；同一 reservation 映射到共享启动 promise，而非共享可独立释放的 owner token；所有 finally/catch 清理做 promise/owner 身份比较后再删除或释放。
- 应补测试：构造超过容量的可阻塞 `createSession()`，逐出压力下重试同一请求；验证只启动一次、失败分支无法关闭成功分支，且旧 promise 清理不影响新 promise。

### REQ-1 请求幂等指纹只包含消息文本，忽略 selected context 与新会话语义（=P2-4，主评审已独立复核）

`electron/application/durable-run-application-service.ts:50-63,116-121` + `electron/application/session-service.ts:229-257` + `shared/domain-state-api.ts:398-426` + `electron/session/session-user-turn-preparer.ts:117-139`：请求缓存与持久化查找的指纹是 `canonicalHash(input.message)`（`:52`/`:116`）——**只覆盖消息文本**。同 session、同 clientRequestId、同消息文本但携带不同附件/选中上下文时，进程内 dedupe 直接返回旧 promise（`:55-62` 只在文本不同时才 CONFLICT，错误文案 "clientRequestId was already used for different message text" 印证口径），重启后 `lookupRequest` 同样只比对文本 hash；而 `context.attachments` 会被准备为 `selected_context` 消息进入 history 与 provider 输入（`session-user-turn-preparer.ts:117-139`），new-session 的 projectId/mode/modelSelection/title/goal 等初始化语义同样不在指纹内。

- 影响：客户端重试或 ID 复用时，用户得到旧 promise/旧结果而非可诊断冲突，实际执行内容与其当前请求不一致。
- 建议：定义版本化的 canonical request fingerprint，覆盖请求类型、消息、规范化 context 与新会话全部语义字段；或明确禁止含 context 的同 ID 重试。指纹持久化，并在内存与重启恢复两条路径一致校验。
- 应补测试：同 ID+文本、不同附件/上下文应冲突；同 ID+文本、不同新会话元数据应冲突；完全相同输入仍稳定复用。

### FC-2 after 重验无大小上限整读文件，主进程内存 DoS 面（=P2-1）

`electron/application/file-change-filesystem.ts:38-47`；调用方 `electron/application/file-change-service.ts:182-191`：`lstat` 已拿到 size（`:38`）但未使用，`readFile(canonical, 'utf8')` 无界整读；commit 的 after 重验与 revert 的两次重验均经过。外部进程把目标替换为超大普通文件即可让 Electron 主进程缓冲整个文件，hash mismatch 只在读完后才发现。工具层的 10 MiB 上限（`electron/tools/file-tool-limits.ts:1`）只作用于 mutation 准备路径，未复用到此。

- 建议：读取前按 size 拒绝超限文件，超限与已变更都稳定报 `RESOURCE_CHANGED`；必须验证大文件时使用有硬上限的流式 hash/读取。
- 应补测试：快照与回滚之间替换为超上限普通文件，验证进程不读取完整内容且不执行写入/删除。

### FC-3 `beforeContent` 与 `beforeHash` 无交叉校验，损坏记录可恢复出错误内容（=P2-2）

`electron/persistence/file-change-codec.ts:166-195` + `electron/application/file-change-service.ts:347-355`：codec 校验 schema、存在性、payload 字节数与 hash 字段格式，但不验证 `sha256(beforeContent) === beforeHash`，也不校验 diff/快照的关联 hash；`revert()` 把 `beforeContent` 直接交给恢复函数，全程不使用 `beforeHash`。DB 损坏、错误导入或部分写入恢复出的「内容被换、hash 格式合法」行会通过全部校验并写回错误内容。裸 SHA-256 只能覆盖意外损坏；若威胁模型包含能修改 DB 的攻击者，还需密钥化完整性保护。

- 建议：至少在 revert 前重算 `sha256(beforeContent)` 与 `beforeHash` 比对，不一致拒绝且不触碰文件；按本地数据库攻击模型评估 HMAC/受保护密钥完整性标签。
- 应补测试：篡改 `before_content`、`before_hash`、payload 长度与 diff 字段的持久化行，验证加载或回滚可靠拒绝且不触碰 workspace 文件。

### FC-4 `UNIQUE(session_id, call_id, path)` 与压缩后 call ID 重用冲突（=P2-5）

`electron/persistence/migrations/0001_initial.sql:189` + `electron/session/canonical-history.ts:267-282` + `electron/application/file-change-service.ts:231-256`：canonical history 只在 `inHistory` 的 assistant turn 中查重 call ID；压缩将旧消息置为非活跃后，后续 provider 可合法重用同一 call ID。重用 ID 对同一路径再次 mutation 时触发文件变更表唯一约束失败 → catch-all 转为 `CHANGE_HISTORY_PERSIST_FAILED` warning：mutation 已成功但本次变更无可回滚记录，用户无感知。

- 建议：统一两端标识语义——全 session 禁止重用 tool call ID，或将数据库唯一键升级为 assistant turn/message identity + call ID + path；不要把「文件已变更但历史未记录」降格为仅日志告警。
- 应补测试：压缩前后使用同一 call ID、同一路径执行 mutation，验证第二条 file change 可持久化并独立回滚，或在执行前返回明确可恢复错误。

### FC-5 revert 不保留权限位，0755 脚本回滚后变 0600（=P2-3）

`electron/application/file-change-filesystem.ts:102-113` + `electron/persistence/file-change-codec.ts:20-23`：恢复以 `0o600` 创建临时文件（`:106`）并直接 rename 覆盖目标（`:113`）；record 只保存回滚前内容，不保存 mode/ACL 等元数据。原本 `0755` 的脚本被删除或覆盖后回滚，内容恢复但失去可执行位；覆盖回滚同样替换目标原权限。

- 建议：record 增加受约束的 `beforeMode & 0o777`，恢复时在 rename 前对临时文件 `fchmod`；ACL、所有者、xattr 与特殊位的跨平台策略单独定义，避免暗中承诺无法恢复的元数据。
- 应补测试：对 `0755` 脚本的 patch/write/delete 做提交及回滚，验证内容与权限都恢复；覆盖非 POSIX 平台兼容分支。

### M-6 高版本/缺字段配置无备份整份重置、secret 孤儿；P5 新增必填字段使其成为直接样本（=P1-3）

`electron/config/store.ts:581-586` + `electron/config/migrations.ts:25-47` + `shared/config.ts:231-234` + `electron/config/schema.ts:148` + `electron/config/store.test.ts:110-125`：`migrateConfig` 对任何 `schemaVersion !== 9`（含 `> 9`）抛 `UnsupportedConfigSchemaError`，`#read` 随即 `rm` 整份配置并写默认值——无 `.bak` 备份；`store.test.ts:354-363` 用 v99 固定了「未来版本也被重置」。P5 将 `limits.fileChangeHistoryBytes` 设为 v9 **必填**字段但未升 schemaVersion，任何缺该字段的 v9 配置（如本分支 `853be30`–`2a9149c` 期间写出的配置）同样整份重置，`store.test.ts:110-126` 正是通过删除这一个字段断言「重置整份配置」。重置路径不触碰 `#secretStore`，旧 `apiKeyRef` 指向的条目成为孤儿永久留在 `secrets.json`。

- 影响：普通升级路径中丢失 providers、MCP servers、remembered permission rules、privacy notices 等全部配置，且无任何恢复途径。**影响面修正（相对 P5 报告 P1-3）**：v9 配置从未发布（release 0.2.1–0.2.3 均为 master v8 产物），当前真实影响仅限分支开发机；但「每个新增必填 limits 字段 = 全量清配置」的模式必须在任何 v9 发布后再次加字段前解决。reset-only 策略已记录于 plan:375 与 p5-checklist:8，但「>9 拒绝保留文件」「重置前备份」「孤儿 secret 清理」三项建议均未实施。
- 建议：为新增字段提供版本化、保字段的迁移（或受控默认值合并）；区分 `schemaVersion > 9`（拒绝并保留文件）与 `< 9`/校验失败（重置）；重置前写入 `.bak`；对被删除的秘密引用做安全、可审计的孤儿清理。
- 应补测试：以 P5 之前的完整 v9 配置为输入，验证仅填充新字段且其余字段逐项保留；分别覆盖损坏 JSON、缺失字段、未知更高版本与迁移失败回滚；补「应保留文件/应备份」的反向断言（当前测试在固化问题）。

### M-8 第 513 个 Project 写库成功但从所有 full-list API 消失（=P2-6）

`shared/durable.ts`（上限 512）+ `electron/persistence/project-repository.ts:73-82` + `electron/application/project-service.ts:75-101` + `shared/domain-state-api.ts:495-509`：repository `list` 固定 `LIMIT 512` 且 `ORDER BY created_at ASC`，`add` 无容量检查——第 513 个（最新）Project 正好被截掉，其 ID 不在 command result、commit event 或 bootstrap 中；契约无 `project:get`/分页，调用方无法再选择、更新或删除它，数据库永久保有 API 不可见的记录。

- 建议：达到上限时于插入前在同一事务中原子拒绝（推荐，符合有界契约），或实施分页与单项目查询并在 UI 完整支持；上限由数据库与服务层共同保证，不能只依赖返回列表截断。
- 应补测试：覆盖第 512/513 个创建，断言后者要么失败且不落库，要么可通过 API 立即发现和管理。

### M-1 reasoning-only / 空 content 的合法 completion 被 `complete()` 误判为错误（=P2-7）

`electron/providers/chat-completions-adapter.ts:335-363`：`MessagePart` 只有 `text | tool_call | tool_result`，`:355-357` 在 `parts.length === 0` 时无条件抛 'Provider completed with an empty assistant turn'。DeepSeek reasoning 模型在 thinking 阶段耗尽 token（`finish_reason: 'length'`、只流出 `reasoning_content`）是真实场景，此时整个 Run 以 confusing error 失败，已流给 UI 的 reasoning 也不落 canonical 记录——重构前旧路径以空答案正常收尾，属行为回退。`parts.minItems=1` + `TextPart.minLength=1` 使「空 text 占位」的简便修法也不可行；compact 路径共用 `complete()`，reasoning-only 压缩响应同样以该报错失败。

- 建议：在 shared assistant schema 中定义受控的 reasoning-only 表示，或在转换层以明确的无内容完成状态处理；不要依赖无意义文本占位符；同步确认 UI、prompt replay 与 trace 的展示策略。
- 应补测试：模拟仅有 reasoning 的 finish、stream completion 与截断响应；断言运行状态、历史、重试语义与用户可见提示一致（当前无任何行为固定，修复时无回归网）。

### M-9 合法附件组合聚合后超单 Message text 上限（=P2-8）

`shared/context.ts:39-44` + `electron/session/session-user-turn-preparer.ts:117-139` + `shared/message.ts:64-69` + `electron/config/schema.ts:143`：RunContext 最多允许 32 个附件，默认每文件 `readFileOutputBytes = 64 KiB`（配置允许提高），全部拼进**一个** `selected_context` text part；`TextPart.text` 上限 1,000,000 字符——约 16 个默认大小文件即可超限，UI 正常操作即可到达。源头无任何聚合上限（`context-attachments.ts:13,53` 只限 32 个；preparer 无大小校验）。

- 影响：durable 组合下由 H-5 恢复兜底（run 以 codec 错误失败但 session 不中毒），但这类正常请求永远无法成功；**legacy 默认组合下超限记录留在内存，此后每次 compile 失败，session 内存态永久不可用直至重启**。
- 建议：在 context 准备层施加聚合 byte/character 总预算，必要时按可解释的优先级截断，或拆分为多条均受上限保护的 context 记录；前端在发送前显示预算与截断信息。
- 应补测试：总大小刚好低于/高于上限的多附件请求——前者成功、后者得到结构化可解释错误或按预期截断，且不写入半条 user turn。

### M-7 `sqlite-smoke.cjs --packaged` 在非 Windows fresh 环境报错而非 skip（确认未修复）

`scripts/sqlite-smoke.cjs:107-110`（主评审抽查确认）：`packagedElectronPath()`（内部 `readdirSync(release/<version>/win-unpacked)`，缺目录即抛 ENOENT）仍在 `process.platform !== 'win32'` skip 判断**之前**求值，底部无 try/catch。fresh 非 Windows 环境独立运行必然非零码崩溃而非输出 `SQLITE_SKIP`，违反 AGENTS.md 声明的 "non-Windows hosts report an explicit skip"；`npm run build` 流程内因 electron-builder 刚产出产物而不触发。建议：平台判断移到路径解析之前；补无 release 目录的非 Windows 用例。
...
- Prompt Inspector CSS grid 仍 6 列 vs 实际 5 个 children（=P3-1，确认未修）：`settings-content.css:289-291` 六个列轨道 vs `LoggingSettingsPanel.vue:240-252` 五个 grid child，source/tokens 落入 48px 窄列被省略号截断，末尾留空列。建议 grid 改为与 5 列对应并在窄宽度下采用可读的堆叠或 tooltip 策略；补长 source/token 值的组件/视觉回归用例。
...
- **SC-5 / M-6 修正**：`fileChangeHistoryBytes` 必填未升版本的影响面见第 6 节 M-6（v9 未发布）；若确认 v9 不随任何中间版本发布，可在 plan 中显式记录「v9 字段可在分支内自由追加」作为决策。
...
- **边界确认**（并入 P5 报告结论）：P5 尚未将 durable target 接入新的 renderer/preload 公共入口；在当前边界内文件快照内容保留在后端持久化路径，未发现 P5 新增的凭据向 renderer、trace 或子进程环境泄漏。
...
- **APP-6**：容量压力下同 key 重入「只启动一次、失败分支无法拆毁成功分支、旧 promise 清理不影响新 promise」无测试。
- **REQ-1**：同 ID+文本不同附件/上下文冲突、同 ID+文本不同新会话元数据冲突、完全相同输入稳定复用，均无测试。
...
- **FC-1/2/3/5**：TOCTOU 注入 hook（目标内容变化 + 父目录换 symlink 两种）、超上限文件替换不整读不写删、篡改 before_content/before_hash/payload 长度/diff 字段可靠拒绝、0755 权限位保留（含非 POSIX 分支）均无测试；revert 的「目标被换成 symlink/目录」分支、`diffTruncated=true` 的 list/revert、retention × in-flight revert 交互无覆盖；经真实 registry 的 revert-vs-run/archive/project-update 并发集成缺失（现仅 mock guard）。
- **FC-4**：压缩后重用 call ID + 同 path 二次 mutation 可持久化并独立回滚（或执行前明确报错）无用例。
- **M-1**：reasoning-only 的 finish/stream/截断响应无行为固定；干净 EOF、usage 模型名、completion 校验失败写 llm.response 均无断言。
- **M-6**：P5 前完整 v9 配置升级仅填充新字段且其余逐项保留；损坏 JSON、缺字段、未知更高版本、迁移失败回滚；secret 孤儿清理断言。
- **M-8**：512/513 project 边界与「add 结果含新 record」原子性。
- **M-9**：聚合刚好低于/高于上限的多附件组合。
...
- **CFG/infra**：fresh-env smoke skip 无测试；`fileChangeHistoryBytes` 边界包含性（恰好 1M / 10G 被接受）未断言；`getProviderApiKeyForRevision` 无直接单测；benchmark `maxStepsPerRun` 封顶与 `MAX_PROVIDER_REQUESTS || 32` 回退无断言。
...
- **P5 骨架**：FileChange commit/event 严格先于 tool-batch Message commit（顺序探针断言）；retention 与 insert 同事务、按 `(created_at,id)` 确定性淘汰、单条超限先拒绝；`payloadBytes` = UTF-8(before)+UTF-8(diff) 双侧重算；Run 起始冻结预算；revert 首个 await 前预留 mutating token、writer lease 双向互斥、不占 run slot；`beforeContent` 不进 shared 契约/page/event/tool result/canonical Message/trace（全文断言）；durable target 不双写 legacy `change-history.json`。
...
- **file-change cursor 分页**：keyset `(created_at, id)` DESC 与 cursor 子句严格对偶、平局确定性有测试；cursor 字段有界且归一化后参数化查询，无注入面；205 条无缺口分页测试。

## 11. 建议处理顺序

整合两份报告后的统一顺序（与 P5 报告 §建议的修复与验证顺序一致，并纳入本轮新发现）：

1. **P6 开工前（结构性/安全项）**：
   - FC-1（TOCTOU，需设计决策：原生 no-follow 原子操作 `openat`/`renameat` 或显式 best-effort 两阶段契约 + 可控测试 hook）；
   - M-6（配置策略决策：>9 拒绝保留、重置前 `.bak`、新增字段保字段迁移、孤儿 secret 策略，用真实旧 v9 fixture 验证无损升级，并文档化）；
   - APP-6（pending/settled 分离 + 共享启动 promise，覆盖容量压力与重试竞态）；
   - PRV-1（reason 截断/校验，补 H-7 缺口）；SR-1（run catch checkpoint 回滚，补 H-5 缺口）；APP-1（update 对齐 archive 策略 + apply 顺序调换）；PER-1（exec 拦截，一行正则即可拆掉 footgun）。
2. **P6/P8 接线前必须修**：FC-2/FC-3/FC-4/FC-5（文件历史读取上限、哈希校验、标识语义、权限元数据，建议作为同一安全/可恢复性改动评审）；REQ-1（请求指纹统一，含重启恢复、压缩和重试组合测试）；M-1（reasoning-only，需 schema 决策）；M-8（add 同事务计数拒绝）；M-9 源头聚合上限；M-7（skip 顺序）；REG 两项（dispose 接线、closeSession 自愈）；SR-5（harness run resolve 通道，若 P6 接入）；SC-3（plan §10.4 同步）；SC-2（file-change 契约冗余 sessionId 在注册前定型）。
3. **随手修**：第 7 节各 Low（unhandled rejection、publish listener 隔离、fork 标题、服务层输入校验、trace 对称性、CSS grid、错误文案等）。
4. **补测试**：按第 9 节优先级——PRV-1/SR-1/APP-1/APP-6/REQ-1 回归、durable 组合取消/auto-compact/interjection、FC 安全组、M-1 行为固定、PER 防护与索引固定。
5. **修复后验证**：至少重复 `npm test`、`npm run lint`、`npm run typecheck`、`npm run test:sqlite`、`npm run test:e2e`；涉及文件系统边界的改动还应在 POSIX 与 Windows 打包运行时各验证一次。

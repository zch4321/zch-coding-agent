...
> 与既有报告的关系：本报告**完整整合**了此前两份评审——P0–P4 评审（覆盖至 `843829a`，H/M 编号体系）与 P5 增量评审（P1/P2/P3 定级）——的全部发现、分析与修复顺序建议，重叠条目保留 P5 增量评审的编号交叉引用（记作 =P1-1 等）；个别定级/事实差异以本报告复核结论为准（见 M-6 影响面修正、APP-6 定级说明）。两份旧文档已被本报告取代并删除，本报告为分支当前唯一全量评审基线。本报告仅评审，未修改任何代码。

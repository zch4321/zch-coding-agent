...
>
> P10 收口：2026-07-26 已完成 transaction authorizer、listener/dispose、FileChange retention totals、retry cache、N-1/N-2、NMessage 通知和单一 `npm run verify` 门禁；P3、N-3/N-4 与 201+ Electron E2E 明确延后。
...
但初审 HEAD 存在 **11 个 P1、30 个 P2、46 个 P3** 问题（无 P0）。P1 中包含四类必须修复的回归，建议修复前不要合入：
...
**截至 `db8072a` 的总结论：11/11 P1 全部修复且均有回归测试；P2 中 28 条修复、2 条部分修复、1 条按决策记录关闭；master 遗留 4 项全部修复；用户实测反馈 5 项中 4 项修复，sendFirst 为有意保留的设计。该次复检另发现 N-1/N-2 与 N-3/N-4，后续 P10 状态如下。**

## P10 收口结果（2026-07-26）

- 原报告 31 条 P2 已全部关闭：30 条通过代码/测试修复，M-6 通过“当前无用户、未发布 v8→v9 不做保字段迁移”的产品决策关闭。`PersistenceTransaction.prepare('COMMIT')` 等路径由 SQLite authorizer 拒绝，并覆盖 COMMIT/ROLLBACK/BEGIN/SAVEPOINT 变体。
- N-1 已修复：carryover 启动失败会移除 FIFO 项和 overlay chip，显示 10 秒 warning，继续下一项并始终解除 starting 状态；不提供重试。N-2 已修复：rejected retry promise 自动从缓存淘汰，成功结果继续幂等缓存，Session eviction 清理 start/retry 两类缓存。
- Durable commit listener 已逐项隔离；backend dispose 使用共享幂等 promise，按 live runtime → coordinator → listeners → database 排空。FileChange retention 改由 `0003_file_change_retention_totals.sql` 的单行总量与 trigger 维护。
- 操作反馈已统一为 NMessage；新增安全 `app:notification`、preload 64 条缓存、warning/error 时长、5 条排队、去重、后台 Session 标题与脱敏测试。ConversationTimeline 不再承载全局 warning/error。
- CI/Release 与本地完整门禁统一为 `npm run verify`；它不包含 benchmark preset、Docker worker/image、外部 benchmark 或真实 Provider。确定性 benchmark manifest/checksum/路径安全测试仍由默认 `npm test` 覆盖。
- 明确延后：46 条 P3 建议、N-3、N-4 和 201+ 数据量的额外 Electron E2E。201+ Session/Message/FileChange 的 store/repository 单元与集成覆盖保留；延后的是更慢的 UI 端到端层。
...
- **按决策记录关闭（1 条）**：config v9 无备份 reset 代码未改；P10 再次确认当前没有用户，v8→v9 不做保字段迁移、备份或孤儿 secret 清理。重新评估点改为首个 v9 版本实际发布之后的下一次不兼容变更。
...
### 修复引入的新问题（复检发现；N-1/N-2 已在 P10 关闭）

- **N-1 [P2，已解决]**：carryover flush 失败曾永久锁死会话输入。P10 现会出队、移除 chip、发 warning、继续 FIFO 且 finally 解锁。
- **N-2 [P2，已解决]**：`retry()` 曾缓存 rejected promise。P10 现只保留成功幂等结果，拒绝后自动淘汰相同 key。
...
- application：`application-error` 误标、idle 守卫不看 `mutationInProgress`；listener 异常隔离与 `dispose()` 排空已由 P10 修复。
...
- **config v9 reset-only 路径无备份、无用户提示、secret 引用成孤儿（P10 决策关闭）**：`electron/config/store.ts` 对不兼容/损坏配置直接 reset。P10 已确认当前没有用户，v8→v9 不做保字段迁移、备份或孤儿 secret 清理；首个 v9 版本实际发布后，后续不兼容变化必须升版本并重新设计迁移与备份。
...
| M-6：未发布 AppConfig v8→v9 的 reset-only 策略           | P10 再确认当前无用户，不迁移/备份/清孤儿 secret；首个 v9 实际发布后再评估    |
...
6. **P2 收尾（P10 已完成）**：project 512 容量语义、transaction authorizer、usage 归因、M-6 决策、trace 清理和启动崩溃恢复均已关闭。
...
1. **`prepare('COMMIT')` 提前提交（P10 已修复）**：事务 callback 现由 SQLite authorizer 拒绝 transaction/savepoint control opcode；COMMIT、END、ROLLBACK、BEGIN、SAVEPOINT、RELEASE 和 ROLLBACK TO 均有 rollback 回归。
...
4. **发布循环无 per-listener 守卫（P10 已修复）**：当前逐个 listener `try/catch` 并记录安全诊断；单个订阅者异常不会阻断后续投递。

**仍开着的功能性不一致**：

- **N-1（P10 已修复）**：carryover flush 失败会同时移除队列项与 UI overlay，发 warning 后继续下一项，不再锁死输入。
...
**当前结论**：P10 已封堵事务控制 SQL、listener 隔离和 N-1/N-2。仍延后的结构性通道是 `PersistenceReader` 可写与 parent-clearing trigger，另有 `agent-project` 竞态和 `closeRuntimeSession` 死 stub；它们按用户决策作为 P3 分块讨论，不属于本轮发布门禁。

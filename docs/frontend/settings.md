...
设置保存先同步捕获完整草稿，再进入所属领域的保存队列。成功响应更新已保存基线，仅当当前草稿仍与提交快照一致时回填表单。Permissions、Limits、Agents 的自动保存会继续提交期间的新编辑，退出时的 flush 等待同一队列；Network、Web Search 和 Assistant 偏好的手动保存保留较新的草稿，后续显式保存可排入同一队列。失败保留输入并释放 saving 状态，允许重试。

排队的显式保存同样在触发时捕获草稿，待执行请求只保留最近一次显式保存的快照，相同内容合并。排队后继续输入的内容保留为未保存草稿，不随旧请求提交；语言切换触发的偏好保存遵循相同规则。

Web Search 的凭据与配置按顺序保存，但第一次 await 前已捕获 key、provider、count；凭据回包只更新凭据状态。同一保存队列中的快照引用相同已提交 key 时不重复写入凭据。第二步失败时保留配置草稿，已提交的 key 不再重复发送；新输入的替换 key 仍保留。清除凭据仅改变凭据状态，不回填其他字段。语言切换失败恢复最后成功保存的语言，保留偏好文本草稿。
...
- API Key 配置状态、更新和清除。
- Provider 名称、类型、地址、默认模型和模型覆盖均自动保存；Provider 不展示或持久化默认 Reasoning。切换卡片或退出页面前排空当前保存，失败时留在当前草稿并显示错误。模型表单与新增模型对话框的容量缺省值只读取已提交的 Runtime 限额；Limits 每次保存成功后立即更新这些默认值，保留 Provider 未保存的表单字段和模型显式覆盖。
...
- Token 估算由 Limits 领域单独保存：默认保守估算，可切换为自定义 `bytesPerToken`；仅影响预算估算，不改变字节/行数硬限制。

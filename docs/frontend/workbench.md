...
保留 textarea，支持文件选择、拖放、截图粘贴和 Windows 文件剪贴板。输入区通过统一的 `+` 菜单提供添加附件、添加文件上下文和添加目录上下文；运行期间附件入口仍可用，工作区上下文选项禁用。附件使用 Naive UI 图片预览、文件卡片与有界滚动区，输入框显示紧凑版本；发出后保留在对应用户消息内，可重新附加。页头布局不变，已有 `@文件` context 选择独立保留。

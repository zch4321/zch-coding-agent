...
保留 textarea，支持文件选择、拖放、截图粘贴和 Windows 文件剪贴板。输入区通过统一的 `+` 菜单提供添加附件、添加文件上下文和添加目录上下文；运行期间附件入口仍可用，工作区上下文选项禁用。附件使用 Naive UI 图片预览、文件卡片与有界滚动区，输入框显示紧凑版本；发出后保留在对应用户消息内，可重新附加。图片采用无边框的横向缩略图，不显示文件名和大小，点击放大；移除和重新附加使用角上的图标，历史图片的操作在悬停或键盘聚焦时显示。页头布局不变，已有 `@文件` context 选择独立保留。
...
工作区文件/目录通过 `+` 菜单或 `@` 补全选择后，以 `@{src/main.ts}`、`@{src/components/}` 插入正文光标处。占位符是普通文本，支持复制、编辑和原生撤销；删除即取消引用。空格和中文直接保留，路径中的花括号以 `\{`、`\}` 转义。目录补全允许继续展开子项，再选择当前目录或文件。

发送与用户消息展示复用同一 Markdown 引用规则：代码块、行内代码、转义文字及 Markdown 链接/图片中的示例不展开为文件上下文。手工输入的 `@path` 仍可使用。发送时按类型/路径去重读取，历史在正文原位置显示为行内代码，不重复展示上方标签。旧消息中的独立选择仅在前端投影时追加为引用，持久历史不改写。带工作区引用的草稿在 Run 结束后发送。
...
- 当前上下文以 bytes 展示总大小、分类大小与来源大小，不显示最大上下文长度；分段条展示系统提示、用户输入、编排消息、助手回复、工具定义、工具调用与工具结果的字节占比。
- 用量明细保留 Provider token 指标，切换整个会话或当前/最近运行，展示主对话、子代理、工具审批、上下文压缩和标题生成；各类可展开模型，子代理可展开任务。
...
Run 取消中的停止按钮只在停止请求尚未返回时禁用；请求返回后可以再次点击以重试进程清理。exec 进程只归当前 Run，未清理完时不发布 Run 终态，也不允许开始下一轮，不添加 Background 卡片。
